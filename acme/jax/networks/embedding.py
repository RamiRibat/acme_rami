...
class OAREmbedding(hk.Module):
	...
	def __call__(
		...
		if expand_obs: # [H,W,C] -> [1,H,W,C]
			inputs = inputs._replace(observation=jnp.expand_dims(inputs.observation, axis=0))
		features = self.torso(inputs.observation)  # [T?, B, D]
		if expand_obs:
			features = jnp.squeeze(features, axis=0)

		# Do a one-hot embedding of the actions.
		action = jax.nn.one_hot(inputs.action, num_classes=self.num_actions)  # [T?, B, A]
		...
		embedding = jnp.concatenate([features, action, reward], axis=-1)  # [T?, B, D+A+1]
