...
class DeepAtariTorso(hk.Module):
	...
	def __init__(
		...
		downsampling_strategies:
			Sequence[resnet.DownsamplingStrategy]
...
class DeepIMPALAAtariNetwork(hk.RNNCore):
	...
	def __init__(
		...
		self._embed = embedding.OAREmbedding(
			torso=DeepAtariTorso(use_layer_norm=True),
...
class R2D2AtariNetwork(hk.RNNCore):
	...
	def __init__(
		...
		self._embed = embedding.OAREmbedding(
			torso=DeepAtariTorso(hidden_sizes=[512], use_layer_norm=True),
			num_actions=num_actions
		)
		self._core = hk.LSTM(512) # LSTM > GRU (if long sequence?)
