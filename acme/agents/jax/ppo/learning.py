...
class PPOLearner(acme.Learner):
	...
	def _make_initial_state(self) -> TrainingState:
		...
		initial_params = self._ppo_networks.network.init(key_init)
		initial_opt_state = self._optimizer.init(initial_params)
		# Using float32 as it covers a larger range than int32. If using int64 we
		# would need to do jax_enable_x64.
		...
		init_state = TrainingState(
			params=PPOParams(model_params=initial_params, num_sgd_steps=params_num_sgd_steps),
		...
		if self._normalize_obs:
			obs_norm_params = self._obs_normalization_fns.init()  # pytype: disable=attribute-error
			obs_norm_params = jax.device_put_replicated(obs_norm_params,
														self.local_learner_devices)
			init_state = init_state._replace(obs_normalization_params=obs_norm_params)
	...
	def step(self):
		...
		# print(f"counter: {self._counter.get_counts()}")
		# print(f"actor: {self._counter.get_counts()['actor_steps']} | learner: {self._counter.get_counts()['learner_steps']}")
