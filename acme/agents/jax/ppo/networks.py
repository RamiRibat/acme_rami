...
def make_inference_fn(
	...
	# print(f'ppo.networks.make_inference_fn [evaluation:{evaluation}]')
...
def make_mvn_diag_ppo_networks(
	...
	def entropy(params: MVNDiagParams, key: networks_lib.PRNGKey) -> networks_lib.Entropy:
		del key
		return tfd.MultivariateNormalDiag(
			loc=params.loc, scale_diag=params.scale_diag
		).entropy()
...
def make_tanh_normal_ppo_networks(
	...
	def build_distribution(params: TanhNormalParams):
		# print(f'build_distribution: params.loc={params.loc}')
		# print(f'build_distribution: params.scal={params.scale.shape}')
