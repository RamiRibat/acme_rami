...
def _make_adder_config(
	step_spec: reverb_base.Step, n_step: int,
	table: str
) -> List[sw.Config]:
...
def _as_n_step_transition(
	...
	Args:
	flat_trajectory: A trajectory with n-step rewards and discounts to be
		process.
	...
	return reverb.ReplaySample(
		...
		data=types.Transition(
			...
			next_observation=tree.map_structure(lambda x: x[-1], trajectory.observation),
