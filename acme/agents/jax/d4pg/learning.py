...
class D4PGLearner(acme.Learner):
	...
	def __init__(
		...
		def policy_loss(
			...
			grad_critic = jax.vmap(jax.grad(critic_mean, argnums=2), in_axes=(None, 0, 0))
		...
		def sgd_step(
			...
			# key, key_policy, key_critic = jax.random.split(state.key, 4)
		...
		self._critic_optimizer = critic_optimizer or optax.adam(1e-4)
		self._policy_optimizer = policy_optimizer or optax.adam(1e-4)
	...
	def _make_initial_state(self) -> TrainingState:
		...
		initial_critic_params = self._critic_network.init(key_critic)
		initial_target_policy_params = initial_policy_params
		initial_target_critic_params = initial_critic_params

		# Initialize optimizers.
		initial_policy_opt_state = self._policy_optimizer.init(initial_policy_params)  # pytype: disable=attribute-error
		initial_critic_opt_state = self._critic_optimizer.init(initial_critic_params)  # pytype: disable=attribute-error

		# Create the initial state and replicate it in all devices.
		state = utils.replicate_in_all_devices(
			TrainingState(
	...
	def get_variables(self, names: List[str]) -> List[networks_lib.Params]:
		...
		return utils.get_from_first_device(
