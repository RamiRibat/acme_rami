...
class TrainingState(NamedTuple):
	...
	dual_params: mpo_types.DualParams # for Lagrange constraint
	opt_state: optax.OptState
	dual_opt_state: optax.OptState # for Lagrange constraint
	steps: int
...
class MPOLearner(acme.Learner):
	...
	def __init__(  # pytype: disable=annotation-type-mismatch  # numpy-scalars
		...
		dual_optimizer: Optional[optax.GradientTransformation],# for Lagrange params
		...
		if self._discrete_policy:
			policy_loss_config = (policy_loss_config or mpo_types.CategoricalPolicyLossConfig())
			self._policy_loss_module = discrete_losses.CategoricalMPO(**dataclasses.asdict(policy_loss_config))
		else:
			policy_loss_config = (policy_loss_config or mpo_types.GaussianPolicyLossConfig())
			self._policy_loss_module = continuous_losses.MPO(**dataclasses.asdict(policy_loss_config))
		...
		self._optimizer = optimizer or _get_default_optimizer(learning_rate, grad_norm_clip)
		self._dual_optimizer = dual_optimizer or _get_default_optimizer(dual_learning_rate, grad_norm_clip)
	...
	def _distributional_loss(
		...
		elif self._critic_type == CriticType.CATEGORICAL_2HOT:
		...
		elif self._critic_type == CriticType.CATEGORICAL:
	...
	def _compute_targets(
		...
		# unchanged if, say, the core is an hk.IdentityCore.
		target_state_embedding, _ = self._networks.torso_unroll(target_params, sequence.observation, initial_state)  # [T, ...]

		# Compute the action distribution from target policy network.
		target_policy = self._networks.policy_head_apply(target_params, target_state_embedding)  # [T, ...]
		...
		if self._critic_type == CriticType.CATEGORICAL:
	...
	def step(self):
		...
		with jax.profiler.StepTraceAnnotation('step', step_num=self._current_step):
			...
			if elapsed_time > 0:
				metrics['steps_per_second'] = (self._sgd_steps_per_learner_step / elapsed_time)
			else:
...
def softmax_cross_entropy(
    logits: chex.Array, target_probs: chex.Array
) -> chex.Array:
