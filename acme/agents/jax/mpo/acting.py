...
def make_actor_core(
	...
	def init(key: jax_types.PRNGKey) -> ActorState:
		...
		core_state = mpo_networks.torso.initial_state_fn(params_initial_state, batch_size)
		return ActorState(
			key=next_key,
	...
	def select_action(
		...
		embeddings, core_state = mpo_networks.torso.apply(params.torso, observations, state.core_state)
		...
		# dist_params = mpo_networks.policy_head_apply(params, embeddings)
		# if stochastic:
		# 	actions = tfd.MultivariateNormalDiag(
		# 		loc=dist_params.loc, scale_diag=dist_params.scale_diag
		# 	).sample(seed=key)
		# else:
		# 	actions = tfd.MultivariateNormalDiag(
		# 		loc=dist_params.loc, scale_diag=dist_params.scale_diag
		...
		return actions, ActorState(
			...
			# log_prob=tfd.MultivariateNormalDiag(loc=dist_params.loc, scale_diag=dist_params.scale_diag).log_prob(actions) if store_log_prob else ()
		)
