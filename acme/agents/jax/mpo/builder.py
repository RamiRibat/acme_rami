...
class MPOBuilder(builders.ActorLearnerBuilder):
	...
	def __init__(
		...
		"""Creates a MPO learner, a behavior policy and an eval actor.

		Args:
			config: a config with MPO hps
		"""
		self.config = config
	...
	def make_dataset_iterator(
		...
		dataset = datasets.make_reverb_dataset(
			...
			max_in_flight_samples_per_worker=(2 * self.config.sgd_steps_per_learner_step *
												self.config.batch_size //
												jax.device_count()),
		...
		if self.config.sgd_steps_per_learner_step > 1:
			dataset = dataset.batch(
				self.config.sgd_steps_per_learner_step, drop_remainder=True)
	...
	def make_learner(
		...
		if self.config.use_cosine_lr_decay:
			learning_rate = optax.warmup_cosine_decay_schedule(
				...
				decay_steps=self.config._max_learner_steps
			)
		...
		with chex.fake_pmap_and_jit(not self.config.jit_learner, not self.config.jit_learner):
			learner = learning.MPOLearner(
				...
				use_online_policy_to_bootstrap=(self.config.use_online_policy_to_bootstrap),
				...
				sgd_steps_per_learner_step=self.config.sgd_steps_per_learner_step,
	...
	def make_actor(
		...
		variable_client = variable_utils.VariableClient(
			...
			key=_POLICY_KEY,
			update_period=self.config.variable_update_period, # target <-- online
			device=None
		)
