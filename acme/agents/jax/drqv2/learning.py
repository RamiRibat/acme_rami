...
class DrQV2Learner(core.Learner):
	...
	def __init__(
		...
		iterator: Iterator[reverb.ReplaySample], # iterator
		...
		_update_step = utils.process_multiple_batches(update_step, num_sgd_steps_per_step)
		self._update_step = (jax.jit(_update_step) if jit else _update_step)
