...
for SUITE in ${SUITES[*]}; do
    for LEVEL in ${LEVELS[*]}; do
        ...
        MUJOCO_GL=egl \
        XLA_PYTHON_CLIENT_MEM_FRACTION=0.23 \
        python ../rl_continuous/run_sac.py \
        --acme_id 'v_test' --seed 1 --suite 'control' --level 'trivial' & sleep 15

        MUJOCO_GL=egl \
        XLA_PYTHON_CLIENT_MEM_FRACTION=0.23 \
        python ../rl_continuous/run_sac.py \
        --acme_id 'v_test' --seed 2 --suite 'control' --level 'trivial' & sleep 15
